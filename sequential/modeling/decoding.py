...
def process_logits(logits_BxN, top_k=0, top_p=0.0, temperature=0.0):
  ...
  shapes = attention.get_shape_list(logits_BxN)
...
def inplace_update_i(tensor_BxL, updates_B, i):
  ...
  B = attention.get_shape_list(tensor_BxL)
  batch_size = B[0]
...
def left2right_decode(symbols_to_logits_fn,
  ...
  if beam_size == 1:
    ...
    logit_ls = tf.TensorArray(tf.float32, size=max_decode_len, clear_after_read=True)
    def decode_loop(i, decodes_BxT, cache_BxU_dict, logit_ls, logits_Bx1xV=None):
      logits_Bx1xV = symbols_to_logits_fn(decodes_BxT, cache_BxU_dict, i) 
      logits_BxV = tf.squeeze(logits_Bx1xV, axis=1)
      logit_ls = logit_ls.write(tf.cast(i, tf.int32), logits_Bx1xV)
      ...
      return i + 1, decodes_BxT, cache_BxU_dict, logit_ls, logits_Bx1xV

    def loop_cond(i, decodes_BxT, unused_cache_BxU_dict, logit_ls, logits_Bx1xV):
    ...
    logits_Bx1xV = tf.zeros([batch_size, 1, vocab_size], dtype=tf.float32)   

    _, decodes, _, logit_ls, logits = tf.while_loop(
        loop_cond, decode_loop,
        [tf.constant(0, dtype=dtype), init_dec_BxT, context_BxU_dict, 
         logit_ls, logits_Bx1xV])

    logit_st = tf.transpose(logit_ls.stack(), perm=[1, 0, 3, 2])
    del(logit_ls)
    logit_st = logit_st[:,:,:,-1]
    scores = tf.nn.softmax(logit_st, -1)
    del(logit_st)
    ...
    return tf.cast(beams[:, 0, :], dtype), scores # log_prob_score
