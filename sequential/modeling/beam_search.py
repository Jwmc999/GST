...
def beam_search(symbols_to_logits_fn,
  ...
  """
  shapes = attention.get_shape_list(init_seq_BxT, expected_rank=2)
  ...
  (_, final_alive_seq_BxMxT, final_alive_scores_BxM, _,
   final_finished_seq_BxMxT, final_finished_scores_BxM) =  tf.nest.map_structure(
...
def _update_i(tensor_BxNxT, updates_BxN, i):
  shapes = attention.get_shape_list(tensor_BxNxT, expected_rank=3)
...
def _flatten_beam_dim(tensor_BxMxU):
  shapes = attention.get_shape_list(tensor_BxMxU)
  tensor_BMxU = tf.reshape(tensor_BxMxU, [shapes[0] * shapes[1]] + shapes[2:])
  return tensor_BMxU


def _unflatten_beam_dim(tensor_BMxU, M):
  shapes = attention.get_shape_list(tensor_BMxU)
  tensor_BxMxU = tf.reshape(tensor_BMxU, [shapes[0] // M, M] + shapes[1:])
  return tensor_BxMxU
